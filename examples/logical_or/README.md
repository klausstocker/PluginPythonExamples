...
Vergleiche mit `>=` zu booleschen Werten auswerten und mit `or`
...
`strom_auffaellig`. Verknüpfe diese Werte mit `or` gemäß folgender Regel:

Gib genau dann `True` zurück, wenn **mindestens ein** Messwert seinen Grenzwert erreicht oder überschritten hat. Sind beide Messwerte auffällig, ist das Ergebnis ebenfalls `True`.
...
„Oder“ ist hier einschließend: Auch zwei auffällige Messwerte erfüllen die Bedingung.
...
```bash
python examples/logical_or/answer.py
python -m unittest discover -s examples/logical_or -p "test_*.py"
```
